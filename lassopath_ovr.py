...
def run_lassopath(train_predictors, train_labels, val_predictors, val_labels,
    ...
    return cs, coeffs, biases, train_accuracies, val_accuracies
...
if __name__ == '__main__':
    ...
    parser.add_argument('model_file', type=str, help='Torch .pt file containing trained OVRModel')
    ...
    parser.add_argument('--fold', type=int, default=None,
                        help='The train/val fold to use.')
    ...
    if args.fold is not None:
        config['Loader Kwargs']['fold'] = args.fold
    ...
    if args.spat:
        submodels = [nn_models.CCNNSpatWgt(
            ...
            order=order,
            abs_coeff=False) for _ in range(num_classes)
        ]
    else:
    ...
    cs = np.logspace(args.c_start, args.c_end, num=args.steps)
    all_coeffs = np.empty((num_classes, order, num_filts, len(cs)))
    all_biases = np.empty((num_classes, len(cs)))
    all_train_accs = np.empty((num_classes, len(cs)))
    all_val_accs = np.empty((num_classes, len(cs)))

    for n, (train_loader, val_loader) in enumerate(loader_gen):
        ...
        train_predictors, train_labels, val_predictors, val_labels = preprocess_data(
            model.models[n], train_loader, val_loader, device=device,
            order_rescaling=order_rescaling
        )

        # Reshape predictors so that the shape is [NSAMP, NORDER, NFILTS]
        train_predictors = train_predictors.reshape((-1, order, num_filts))
        val_predictors = val_predictors.reshape((-1, order, num_filts))

        # Manually zero out order-1 features
        ...
        cs, coeffs, biases, train_accs, val_accs = run_lassopath(
            train_predictors, train_labels, val_predictors, val_labels,
        ...
        all_coeffs[n] = np.transpose(coeffs, (1, 2, 3, 0))[0]
        all_biases[n] = biases[:, 0]
        all_train_accs[n] = train_accs
        all_val_accs[n] = val_accs
        if n == num_classes - 1:
            break

    if args.save is not None:
        save_path(
            args.save + '.pkl', cs, all_coeffs, all_biases, all_train_accs, all_val_accs
        )
